...
export async function runPlanMode(): Promise<void> {
  ...
  // generatePlan method hamra passs aa rah hai from planner.ts se 
  const plan = await generatePlan(goal);

  printPlan(plan); // ye method print kare ga sare plans
  ...
  for (const step of selected) {
    ...
    if(r.text) return console.log(renderMarkeddownTerminal(r.text))
