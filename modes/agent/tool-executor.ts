...
// this function basically checks is the given file is a text file or not.
function isProbablyTextFile(filePath: string): boolean {
    const ext = path.extname(filePath).toLowerCase();
    return TEXT_EXT.has(ext) || ext === "";
}

// basically ye jo pura ka pura tool-executor hai ye acts kare ga as a bridge between your AI Tools and a real file system ke becch mein.
export class ToolExecutor {
    // overlay ek tarika ka virtual file system hai 
    private overlay = new Map<string, string>();
    // deleted ka yaha pr simple se matlab ye hai ke jo bhi delete karna chata ho file uska path note kar leya hai basss abhi actuall mein file ko delete nhi kara hai.
    private deleted = new Set<string>();

    // ye norm basically hamri commands ko normalize kar raha hota hai.
    ...
    // ess function ke through humm changes ko accept kar raha hoga jo ai ne staging area pr kare hai.
